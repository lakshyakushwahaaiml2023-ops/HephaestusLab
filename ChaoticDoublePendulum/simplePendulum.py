...
HISTORY_LEN    = 700
wave_points    = ti.Vector.field(2, dtype=ti.f32, shape=HISTORY_LEN)
history        = ti.field(dtype=ti.f32, shape=HISTORY_LEN)
h_head         = ti.field(dtype=ti.i32, shape=())
h_count        = ti.field(dtype=ti.i32, shape=())
...
def record_state():
    ...
    # History wave
    hh = h_head[None]
    history[hh] = theta[None]
    h_head[None] = (hh + 1) % HISTORY_LEN
    if h_count[None] < HISTORY_LEN:
        h_count[None] += 1
...
def draw_history_panel():
    ...
    if cnt > 1:
        max_theta = 3.1415926
        amp = (WAVE_BOTTOM - WAVE_TOP) * 0.5 * 0.85 

        for i in range(cnt):
            # Map index to history array
            idx = (head - cnt + i + HISTORY_LEN) % HISTORY_LEN
            val = history[idx]

            x_val = 1.0 - ti.cast(cnt - 1 - i, ti.f32) / ti.cast(HISTORY_LEN - 1, ti.f32)
            y_val = wave_mid + (val / max_theta) * amp
            wave_points[i] = ti.Vector([x_val, y_val])
...
def main():
    init()
    gui = ti.GUI("Simple Pendulum History Timeline", res=(W, H), fast_gui=False)
